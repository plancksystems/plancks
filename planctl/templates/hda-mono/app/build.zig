...
pub fn build(b: *std.Build) void {
    ...
    const tasks_clean = b.addSystemCommand(&.{ "planctl", "clean", "src/features/tasks/fragments/" });
    const tasks_preprocess = b.addSystemCommand(&.{ "planctl", "src/features/tasks/zsx/", "src/features/tasks/fragments/" });
    tasks_preprocess.step.dependOn(&tasks_clean.step);
    ...
    wasm.entry = .disabled;
    wasm.rdynamic = true;
    wasm.step.dependOn(&tasks_preprocess.step);
    const wasm_install = b.addInstallArtifact(wasm, .{
        .dest_dir = .{ .override = .{ .custom = "wasm" } },
    });

    const dev_exe = b.addExecutable(.{
        .name = "__PROJECT_NAME___dev",
        .root_module = b.createModule(.{
    ...
    dev_exe.step.dependOn(&tasks_preprocess.step);
    ...
    b.step("wasm", "Build WASM bundle").dependOn(&wasm_install.step);
    b.step("dev", "Run native dev server").dependOn(&run_dev.step);
    b.step("dev-build", "Build native dev binary without running").dependOn(&dev_install.step);
