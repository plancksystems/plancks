...
import { exportManifest } from '../api'
...
async function onSubmit() {
  ...
  try {
    ...
    const data = await exportManifest(manifest.value, opts)
    const elapsed = ((Date.now() - start) / 1000).toFixed(1)
    if (data.scheduled) {
      result.value = `Schedule created (${elapsed}s)`
    } else {
      result.value = `${data.message || 'Export complete'} (${elapsed}s)`
    }
    emit('done')
  } catch (e) {
    const elapsed = ((Date.now() - start) / 1000).toFixed(1)
  ...
  <Modal title="Export" @close="emit('close')">
    <div class="space-y-3">
      ...
      <div v-if="preview.store || preview.format" class="p-2 bg-slate-50 rounded border border-slate-200 text-xs space-y-0.5">
        ...
        <div v-if="preview.output_dir" class="flex gap-2"><span class="text-slate-400 w-16">Output:</span><span class="font-mono text-slate-700">{{ preview.output_dir }}</span></div>
        <div v-if="preview.query" class="flex gap-2"><span class="text-slate-400 w-16">Query:</span><span class="font-mono text-slate-700">{{ preview.query }}</span></div>
      </div>
      ...
      <template v-if="mode === 'schedule'">
        <div>
          ...
          <input v-model="scheduleName" type="text" class="w-full px-2 py-1.5 text-xs border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500" placeholder="daily-orders-export" />
        </div>
        <div>
    ...
    <template #footer>
      ...
      <button class="px-3 py-1.5 text-xs bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50" :disabled="loading" @click="onSubmit">
        {{ loading ? 'Working...' : mode === 'schedule' ? 'Create Schedule' : 'Export' }}
      </button>
    </template>
  </Modal>
</template>
