...
import { fetchServiceBackups, createServiceBackup, restoreServiceBackup, deleteServiceBackup } from '../api'
import { HardDrive, Plus, RotateCcw, Trash2, RefreshCw, AlertTriangle } from 'lucide-vue-next'
...
async function loadBackups() {
  ...
  error.value = null
  try {
    const data = await fetchServiceBackups(props.serviceName)
...
async function onCreate() {
  if (!createPath.value.trim()) return
  creating.value = true
  error.value = null
  try {
    await createServiceBackup(props.serviceName, createPath.value.trim())
...
async function onRestore(backup) {
  ...
  error.value = null
  try {
    await restoreServiceBackup(props.serviceName, backup.path || backup.backup_path)
...
async function onDelete(backup) {
  error.value = null
  try {
    await deleteServiceBackup(props.serviceName, backup.path || backup.backup_path)
...
watch(() => props.serviceName, loadBackups)
</script>
...
<template>
  <div class="flex-1 flex flex-col bg-white min-w-0 overflow-hidden">
    <div class="bg-slate-100 border-b border-slate-200 flex items-center px-4 py-2">
      ...
      <span class="text-xs text-slate-400 ml-2">{{ serviceName }}</span>
    ...
    <div class="flex-1 overflow-y-auto p-4">
      <div v-if="showCreateForm" class="mb-4 p-3 bg-slate-50 border border-slate-200 rounded">
        <div class="text-xs font-medium text-slate-600 mb-2">Create Backup</div>
        <div class="flex gap-2">
          <input
            ...
            placeholder="/backups/my-service/2024-01-15"
      ...
      <div v-if="backups.length > 0" class="space-y-2">
        ...
        >
          <div class="flex items-center justify-between">
            <div>
              <div class="text-xs font-medium text-slate-700 font-mono">{{ backup.path || backup.backup_path || backup.name || 'Backup ' + idx }}</div>
              <div class="text-[10px] text-slate-400 mt-0.5 flex gap-3">
                <span v-if="backup.timestamp">{{ formatDate(backup.timestamp) }}</span>
                <span v-if="backup.size_bytes">{{ formatSize(backup.size_bytes) }}</span>
                <span v-if="backup.vlog_count">{{ backup.vlog_count }} vlogs</span>
                <span v-if="backup.entry_count">{{ backup.entry_count.toLocaleString() }} entries</span>
              </div>
            </div>
      ...
      <div v-else-if="!loading" class="text-center py-12">
        ...
        <p class="text-sm text-slate-400">No backups found</p>
        <p class="text-xs text-slate-400 mt-1">Create a backup to get started</p>
      </div>
    </div>

    <div v-if="restoreTarget" class="fixed inset-0 bg-black/30 flex items-center justify-center z-50" @click.self="restoreTarget = null">
      <div class="bg-white rounded-lg shadow-xl p-4 w-96">
        <div class="flex items-center gap-2 text-amber-600 mb-3">
          <AlertTriangle :size="18" />
          <span class="text-sm font-medium">Confirm Restore</span>
        </div>
        <p class="text-xs text-slate-600 mb-2">
          This will <strong>stop</strong> the service, restore from backup, then <strong>restart</strong> it. Any data written after this backup will be lost.
        </p>
        <div class="text-xs text-slate-500 font-mono bg-slate-50 p-2 rounded mb-3">
          {{ restoreTarget.path || restoreTarget.backup_path }}
        </div>
        <div class="flex justify-end gap-2">
          <button class="px-3 py-1.5 text-xs text-slate-600 hover:bg-slate-100 rounded" @click="restoreTarget = null">Cancel</button>
          <button
            class="px-3 py-1.5 text-xs bg-amber-600 text-white rounded hover:bg-amber-700 disabled:opacity-50"
            :disabled="restoring"
            @click="onRestore(restoreTarget)"
          >
    ...
    <div v-if="deleteTarget" class="fixed inset-0 bg-black/30 flex items-center justify-center z-50" @click.self="deleteTarget = null">
      <div class="bg-white rounded-lg shadow-xl p-4 w-96">
        ...
        <div class="text-xs text-slate-500 font-mono bg-slate-50 p-2 rounded mb-3">
          {{ deleteTarget.path || deleteTarget.backup_path }}
        </div>
        <div class="flex justify-end gap-2">
